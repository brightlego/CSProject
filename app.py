    ...
    if flask.request.method == 'POST':
        try:
            ...
            return flask.render_template('login.html',
                                         error_message="Username or Password is"
                                                       " incorrect")
        ...
        try:
            ...
            if username == "":
                return flask.render_template('signup.html',
                                             error_message="Username must "
                                                           "contain at least "
                                                           "1 character")
            if len(password) < MIN_PASSWORD_LENGTH:
                return flask.render_template('signup.html',
                                             error_message="Password must be at"
                                                           " least 3 characters"
                                                           " long")
            if password != password_again:
                return flask.render_template('signup.html',
                                             error_message="Passwords do not"
                                                           " match")
...
def calculator(filepath=""):
    ...
    if filepath and flask.request.method == 'GET':
        try:
            filename, description, raw_text = cloud_storage.get_one_file(
                filepath)
        except client_cloud_storage.AuthorisationError:
    ...
    if flask.request.method == 'POST' and flask.request.form['for'] == 'save':
        ...
        if filename == "":
            error_message = "File name too short. It should be at least one " \
                            "character"
        ...
        if filepath == "":
            ...
            try:
                cloud_storage.update_file(filepath, filename, description,
                                          content)
            except client_cloud_storage.AuthorisationError:
    ...
    if flask.request.method == 'POST' and flask.request.form['for'] == 'graph':
        ...
        path, error_message = evaluator.evaluate(raw_text, width, height,
                                                 (minx, maxx), (miny, maxy))
