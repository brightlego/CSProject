...
OPERATORS = {'i': evaluator.builtins.types.Number(1j), 'pi': evaluator.builtins.types.Number(np.pi), 'e': evaluator.builtins.types.Number(np.e)}
...
def integrate(params):
    ...
    if not isinstance(expression, evaluator.execution_tree.ExecutionTree._ExecutionTree__Node):
        raise ValueError
    ...
    total *= 2
    total -= executor.evaluate_function(expression, low.value, identifier.name.item)
    total -= executor.evaluate_function(expression, high.value, identifier.name.item)
    integral = total * dx/2
...
def differentiate(params):
    ...
    dx = 0.0000001
    result = (executor.evaluate_function(expression, value.value, identifier.name.item) - executor.evaluate_function(expression, value.value - dx, identifier.name.item)) / dx
